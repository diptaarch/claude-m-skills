...
### 2. Extract ClickUp Ticket Context (if applicable)

Parse the current branch name to find ClickUp ticket IDs. Ticket context can be seen on the branch name as well using these patterns:
- `CU-123-feature-name`
- `feature/CU-123-description`
- `CU-123`

If a ClickUp ticket ID is found:
- Fetch ticket details from the branch name context
- Get title, description, acceptance criteria from available context
...
These commands are intentionally NOT in the alwaysAllow list, so the user will be prompted for approval.
